<?php if (!defined('THINK_PATH')) exit(); /*a:5:{s:49:"/var/www/card.com/app/admin/view/public/base.html";i:1502609788;s:53:"/var/www/card.com/app/admin/view/public/head_out.html";i:1502609788;s:50:"/var/www/card.com/app/admin/view/public/alert.html";i:1502609788;s:52:"/var/www/card.com/app/admin/view/public/sidebar.html";i:1502609788;s:48:"/var/www/card.com/app/admin/view/public/nav.html";i:1502609788;}*/ ?>
<!DOCTYPE html>
<html lang="en">
<head>
    <!-- META SECTION -->
...
<div class="animated fadeIn page-container page-navigation-top-fixed">
    ...
    <ul class="x-navigation x-navigation-custom">
        ...
        <?php if(is_array($menuAuth) || $menuAuth instanceof \think\Collection || $menuAuth instanceof \think\Paginator): if( count($menuAuth)==0 ) : echo "" ;else: foreach($menuAuth as $key=>$v): if(empty($v['child']) || (($v['child'] instanceof \think\Collection || $v['child'] instanceof \think\Paginator ) && $v['child']->isEmpty())): ?>
                <li  class="<?php if(in_array(($v['id']), is_array(menu_nav_list())?menu_nav_list():explode(',',menu_nav_list()))): ?>active<?php endif; ?> active_li">
            ...
            <?php else: ?>
                <li class="<?php if(in_array(($v['id']), is_array(menu_nav_list())?menu_nav_list():explode(',',menu_nav_list()))): ?>active<?php endif; ?> xn-openable">
                    ...
                    <ul>
                        <?php if(is_array($v['child']) || $v['child'] instanceof \think\Collection || $v['child'] instanceof \think\Paginator): if( count($v['child'])==0 ) : echo "" ;else: foreach($v['child'] as $key=>$v1): if(empty($v1['child']) || (($v1['child'] instanceof \think\Collection || $v1['child'] instanceof \think\Paginator ) && $v1['child']->isEmpty())): ?>
                            <li class="<?php if(in_array(($v1['id']), is_array(menu_nav_list())?menu_nav_list():explode(',',menu_nav_list()))): ?>active<?php endif; ?> active_li"><a href="#" data-url="<?php echo url($v1['module'].'/'.$v1['controller'].'/'.$v1['method']); ?>" onclick="turn_url(this)"><span class="<?php echo $v1['icon']; ?>"></span><?php echo $v1['name']; ?></a></li>
                           <?php else: ?>
                             <li><a href="#"><span class="<?php echo $v1['icon']; ?>"></span><?php echo $v1['name']; ?></a>
                                <ul>
                                    ...
                                    <li  class="<?php if(in_array(($v1['id']), is_array(menu_nav_list())?menu_nav_list():explode(',',menu_nav_list()))): ?>active<?php endif; ?> active_li"><a href="#" data-url="<?php echo url($v['module'].'/'.$v['controller'].'/'.$v['method']); ?>" onclick="turn_url(this)"><span class="<?php echo $v2['icon']; ?>"></span><?php echo $v2['name']; ?></a></li>
...
<script type="text/javascript">
    ...
    <!-- MESSAGES -->
    <li class="xn-icon-button pull-right">
        <a href="#" title="<?php echo $self['user_login']; ?>【编辑】" data-url="<?php echo url('admin/Admin/edit_self_page',['id'=>$self['id']]); ?>" onclick="edit_row(this)"><span class="fa fa-user"></span></a>
        <div class="informer informer-danger"><?php echo $self['user_hits']; ?></div>
    </li>
    <li class="xn-icon-button pull-right">
        <?php echo get_rank($self['user_rank']); ?>
    </li>
